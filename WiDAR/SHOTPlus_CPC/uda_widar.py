...
import network
import rotation
from cpc_networks import CPCModel, init_weights
...
def test_target_baseline(output_dir, out_file, random_state, device, use_parallel, source_model_dir=None):
    ...
    # Use source_model_dir if provided, otherwise fall back to output_dir
    model_dir = source_model_dir if source_model_dir is not None else output_dir
    load_model_weights(netF, osp.join(model_dir, "source_F.pt"), device)
    load_model_weights(netB, osp.join(model_dir, "source_B.pt"), device)
    load_model_weights(netC, osp.join(model_dir, "source_C.pt"), device)
...
# CPC pre-training is now handled by cpc_utils.pretrain_cpc()
...
def train_target_rot(output_dir, out_file, dset_loaders, netF, netB, device, use_parallel):
    ...
    # netF and netB should already be loaded with source weights by the caller
...
def train_target_shot(output_dir, out_file, random_state, device, use_parallel, source_model_dir=None):
    ...
    # Initialize auxiliary task results
    acc_rot = None
    acc_cpc = None

    # Load pre-trained source model first (needed for both rotation and CPC pre-training)
    ...
    # Rotation SSL network (if enabled)
    netR = None
    if preset["training"]["ssl"] > 0:
        ...
        print(f"Rotation SSL pre-training completed! Best accuracy: {acc_rot:.2f}%")

    # CPC Self-supervised learning (if enabled)
    cpc_model = None
    if preset["training"].get("cpc_weight", 0) > 0 and preset["training"].get("cpc_pretrain", False):
        ...
        cpc_model = CPCModel(
            input_freq=var_x_shape[1] if len(var_x_shape) > 1 else 180,  # F dimension (frequency bins)
            ...
            num_tx=6,  # 6 transmitters
            num_subcarrier=30,  # 30 subcarriers
            window_size=preset["training"].get("cpc_window_size", 10),
        ...
        best_cpc_state, best_cpc_acc = pretrain_cpc(
            cpc_model=cpc_model,
            data_loader=dset_loaders["target"],
            device=device,
            num_epochs=preset["training"].get("cpc_pretrain_epochs", 50),
            lr=preset["training"].get("cpc_pretrain_lr", 1e-3),
            out_file=out_file
        )
        # Load best CPC weights
        cpc_model.load_state_dict(best_cpc_state)
        acc_cpc = best_cpc_acc
        print(f"CPC pre-training completed! Best accuracy: {best_cpc_acc:.4f}")

    # Reload source model and enable gradients for adaptation
    print("Reloading source model for adaptation...")
    load_model_weights(netF, osp.join(model_dir, "source_F.pt"), device)
    load_model_weights(netB, osp.join(model_dir, "source_B.pt"), device)
    load_model_weights(netC, osp.join(model_dir, "source_C.pt"), device)
    ...
    # Setup optimizer - only for netF, netB, and optionally netR and CPC model
    param_group = []
    for k, v in get_model_device_safe(netF).named_parameters():
        param_group += [{'params': v, 'lr': preset["training"]["adaptation_lr"]}]
    for k, v in get_model_device_safe(netB).named_parameters():
    ...
    if netR is not None:
        ...
        netR.train()
    if cpc_model is not None:
        for k, v in get_model_device_safe(cpc_model).named_parameters():
            param_group += [{'params': v, 'lr': preset["training"]["adaptation_lr"]}]
        cpc_model.train()
    ...
    while iter_num < max_iter:
        ...
        # === ROTATION SSL LOSS ===
        if netR is not None and preset["training"]["ssl"] > 0:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if val_acc >= best_val_acc:
                ...
                if preset["training"]["save_models"]:
                    ...
                    if cpc_model is not None and preset["training"].get("cpc_weight", 0) > 0:
                        savename += f'_cpc_{preset["training"]["cpc_weight"]}'
                    ...
                    if cpc_model is not None:
                        save_model_weights(cpc_model, osp.join(output_dir, f"target_CPC_{savename}_best.pt"), use_parallel)
    ...
    # SSL and CPC Pre-training Results
    if acc_rot is not None or acc_cpc is not None:
        ...
        if acc_cpc is not None:
            log_str += f'CPC Accuracy: {acc_cpc:.4f}\n'
        log_str += '\n'
...
def run_single_experiment(run_idx, seed, base_output_dir, source_model_base_dir):
    ...
    source_model_dir = osp.join(source_model_base_dir, f"run_{run_idx}")
    ...
    # PHASE 1: Source Model Loading (NOT training - we load from SHOTPlus)
    print("\n" + "="*60)
    print("PHASE 1: LOADING SOURCE MODEL FROM SHOTPlus")
    print("="*60)
    print(f"Loading source models from: {source_model_dir}")

    # Check if source models exist
    required_models = ['source_F.pt', 'source_B.pt', 'source_C.pt']
    missing_models = [m for m in required_models if not osp.exists(osp.join(source_model_dir, m))]

    if missing_models:
        error_msg = f"Error: Required source models not found in {source_model_dir}\n"
        error_msg += f"Missing: {', '.join(missing_models)}\n"
        error_msg += "Please train the SHOTPlus model first before running SHOTPlus_CPC."
        raise FileNotFoundError(error_msg)

    # Try to load existing source results from JSON
    source_results = None
    source_results_file = osp.join(source_model_dir, 'source_training_results.json')
    ...
    if source_results is None:
        ...
        source_results = {
            ...
            'test_hamming_loss': 0.0,
            'training_history': [],
            'note': 'Source model loaded from SHOTPlus checkpoint'
        }
    ...
    # PHASE 3: Target Adaptation (SHOT + Rotation + CPC)
    print("\n" + "="*60)
    print("PHASE 3: TARGET DOMAIN ADAPTATION (SHOT + Rotation + CPC)")
    print("="*60)
    savename = f'par_{preset["training"]["cls_par"]}'
    if preset["training"].get("ssl", 0) > 0:
        savename += f'_ssl_{preset["training"]["ssl"]}'
    if preset["training"].get("cpc_weight", 0) > 0:
        savename += f'_cpc_{preset["training"]["cpc_weight"]}'
...
def main():
    ...
    print("\n" + "="*80)
    print("SHOT + ROTATION SSL + CPC DOMAIN ADAPTATION EXPERIMENT")
    print("="*80)
    ...
    # Each method has its own WiDAR_Domain_Adaptation_results subdirectory
    ...
    # Go up to WiDAR directory, then into SHOTPlus directory
    ...
    shotplus_scenario_name = scenario_name.replace("_CPC_Rotation", "").replace("_CPC", "").replace("_Rotation", "")
    ...
    print(f"Method: SHOT + Rotation SSL + CPC (using source models from SHOTPlus)")
    print(f"\nResolved Paths:")
    ...
    print(f"\nFull Paths:")
    print(f"  CPC output: {base_output_dir}")
    print(f"  SHOTPlus source: {source_model_base_dir}")

    # Verify source model base directory exists
    if not osp.exists(source_model_base_dir):
        print(f"\nWARNING: Source model directory does not exist!")
        print(f"  Expected: {source_model_base_dir}")
        print(f"Please ensure SHOTPlus has been trained before running SHOTPlus_CPC.")
    else:
        print(f"\n✓ Source model directory verified: {source_model_base_dir}")
    print()
    ...
    for i in range(preset["repeat"]):
        ...
        try:
            final_metrics = run_single_experiment(i, current_seed, base_output_dir, source_model_base_dir)
            if final_metrics:
