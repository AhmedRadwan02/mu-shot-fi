...
def _rot_tf(x: torch.Tensor, k: int):
    """
    Rotate in the (T, F) plane by k*90 degrees.
    Handles both:
    - x: (..., T, F, C) - channels last (4D or more)
    - x: (..., T, F) - no channel dimension (3D or less)
    """
    # Determine rotation dims based on tensor shape
    # For (..., T, F, C): rotate on dims (-3, -2) = (T, F)
    # For (..., T, F): rotate on dims (-2, -1) = (T, F)
    if x.dim() >= 4 or (x.dim() == 3 and x.shape[-1] <= 2):
        # 4D+ tensor (B, T, F, C) or 3D with small last dim (T, F, C)
        dims = (-3, -2)
    else:
        # 3D tensor (B, T, F) or 2D (T, F)
        dims = (-2, -1)
...
def rotate_single_with_label(img: torch.Tensor, label: int):
    """
    Apply rotation based on label.
    img: (T, F, C) or (T, F) tensor
...
def rotate_batch_with_labels(batch: torch.Tensor, labels: torch.Tensor):
    """
    Apply rotation to batch based on labels.
    batch: (B, T, F, C) or (B, T, F) OR (T, F, C) or (T, F) single sample
...
def rotate_batch_with_labels(batch: torch.Tensor, labels: torch.Tensor):
    """
    Apply rotation to batch based on labels. Handles numpy or torch labels.
    batch: (B, T, F, C) or (B, T, F) OR (T, F, C) or (T, F) single sample
